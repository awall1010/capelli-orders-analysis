...
# data_file_current_season = os.path.join('shippingdates', 'aggregated_orders8.10.csv')
...
@st.cache_data
def load_historical_2023_2024_data(filepath: str, bust: float):
    logger.info(f"Attempting to load historical data from: {filepath}")
    p = Path(filepath)
    if not p.exists():
        raise FileNotFoundError(
            f"Missing: {repr(p.name)} in {repr(str(p.parent))} | available={ [repr(x.name) for x in p.parent.iterdir()] }"
        )
    df_hist = pd.read_csv(p, sep=None, engine='python', on_bad_lines='warn')
    if df_hist.shape[1] == 1 and df_hist.shape[0] > 0:
        df_hist = pd.read_csv(p, sep=',', engine='python', on_bad_lines='warn')
    return df_hist

@st.cache_data
def load_current_season_data(filepath: str, bust: float):
    logger.info(f"Attempting to load current season data from: {filepath}")
    p = Path(filepath)
    if not p.exists():
        raise FileNotFoundError(
    ...
    df_current = pd.read_csv(p, sep=None, engine='python', on_bad_lines='warn')
    if df_current.shape[1] == 1 and df_current.shape[0] > 0:
        df_current = pd.read_csv(p, sep=',', engine='python', on_bad_lines='warn')
    return df_current
...
DATA_DIR = (APP_DIR / "shippingdates").resolve()

HIST_PATH = DATA_DIR / "Capelli2023_and2024_aggregated_data.csv"
CURR_PATH = DATA_DIR / "aggregated_orders8.10.csv"

# Debug info to verify on Streamlit Cloud
st.write(f"📁 Data dir: {DATA_DIR}")
st.write("📄 Files:", [p.name for p in DATA_DIR.glob("*")])

hist_bust = HIST_PATH.stat().st_mtime if HIST_PATH.exists() else time.time()
curr_bust = CURR_PATH.stat().st_mtime if CURR_PATH.exists() else time.time()

try:
    df_historical_data = load_historical_2023_2024_data(str(HIST_PATH), hist_bust)
except FileNotFoundError as e:
    st.error(str(e))
    st.stop()

try:
    df_current_season_data = load_current_season_data(str(CURR_PATH), curr_bust)
except FileNotFoundError as e:
    st.error(str(e))
    st.stop()
