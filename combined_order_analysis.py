...
st.write("""
This application analyzes and compares aggregated order data from the `Capelli2023_aggregated_orders.csv` and `aggregated_orders3.17.csv` files. Explore delivery times and shipping performance across different clubs based on the order creation dates.
""")
...
def load_recent_data(filepath):
    ...
    Parameters:
    - filepath (str): Path to the aggregated_orders3.17.csv file.
...
data_file_recent = os.path.join('shippingdates', 'aggregated_orders3.17.csv')
...
st.markdown("---")
st.write("**Note:** This analysis is based on the data available in the `Capelli2023_aggregated_orders.csv` and `aaggregated_orders3.17.csv` files. Please ensure the data is up-to-date for accurate insights.")
