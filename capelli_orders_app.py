...
# -------------------------- Display Percentage and Total Open Orders Over 5 Weeks Old -------------------------- #

st.subheader("Percentage and Total Open Orders Over 5 Weeks Old by Report Date")
st.write("""
This table provides both the **percentage of orders over five weeks old** and the **total number of open orders over five weeks old** for each report date.
""")

# Create a DataFrame with necessary columns
percentage_total_df = ship_and_open_df[['Report Date', 'Total Open Orders Over 5 Weeks', 'Percentage Open Over 5 Weeks']].copy()
percentage_total_df['Report Date'] = percentage_total_df['Report Date'].dt.strftime('%Y-%m-%d')
percentage_total_df['Total Open Orders Over 5 Weeks'] = percentage_total_df['Total Open Orders Over 5 Weeks'].astype(int)
percentage_total_df['Percentage Open Over 5 Weeks'] = percentage_total_df['Percentage Open Over 5 Weeks'].round(2)

# Rename columns for clarity
percentage_total_df.rename(columns={
    'Total Open Orders Over 5 Weeks': 'Total Open Orders Over 5 Weeks',
    'Percentage Open Over 5 Weeks': 'Percentage of Open Orders Over 5 Weeks Old (%)'
}, inplace=True)

# Apply formatting
styled_percentage_total = percentage_total_df.style.format({
    'Total Open Orders Over 5 Weeks': "{:,}",
    'Percentage of Open Orders Over 5 Weeks Old (%)': "{:.2f}%"
})

# Display the styled table
st.write(styled_percentage_total)
...
# -------------------------- New Section: Percentage of Open Orders Over 5 Weeks Old by Club and Report Date -------------------------- #
...
# Pivot the table to have Report Dates as columns and Clubs as rows
percentage_pivot = percentage_per_club_date.pivot(index='Club', columns='Report Date', values='Percentage Over 5 Weeks Old').fillna(0).astype(float)
...
# Convert report date columns to string format for easier formatting
percentage_pivot.columns = ['Club'] + [col.strftime('%Y-%m-%d') for col in percentage_pivot.columns[1:]]

# Reorder columns to have 'Club' first, followed by sorted report dates
percentage_pivot = percentage_pivot[['Club'] + sorted_report_dates_strings]

# Apply formatting to percentage columns
styled_percentage_pivot = percentage_pivot.style.format("{:.2f}%", subset=sorted_report_dates_strings)
...
# st.write(styled_pivot)
#
# # -------------------------- New Section: Percentage of Orders Over 5 Weeks Old and Shipped Back per Club -------------------------- #
#
# st.subheader("Percentage of Orders Over 5 Weeks Old and Shipped Back per Club")
# st.write("""
# This table displays, for each club, the **percentage of orders over five weeks old** and the **number of orders shipped back (returns)**.
# """)
#
# # Calculate total orders per club
# total_orders_per_club = data.groupby('Club')['Order ID'].nunique().reset_index(name='Total Orders')
#
# # Calculate 'Outstanding Over 5 Weeks' orders per club
# outstanding_orders_per_club = data[data['Order Category'] == 'Outstanding Over 5 Weeks'].groupby('Club')['Order ID'].nunique().reset_index(name='Outstanding Over 5 Weeks')
#
# # Calculate 'Shipped Back' orders per club (assuming 'Combined Order Status' == 'returned' indicates a shipped back order)
# shipped_back_orders_per_club = data[data['Combined Order Status'] == 'returned'].groupby('Club')['Order ID'].nunique().reset_index(name='Shipped Back Orders')
#
# # Merge all metrics into a single DataFrame
# percentage_shipped_back_df = pd.merge(total_orders_per_club, outstanding_orders_per_club, on='Club', how='left')
# percentage_shipped_back_df = pd.merge(percentage_shipped_back_df, shipped_back_orders_per_club, on='Club', how='left')
#
# # Fill NaN values with 0
# percentage_shipped_back_df.fillna({'Outstanding Over 5 Weeks': 0, 'Shipped Back Orders': 0}, inplace=True)
#
# # Calculate percentage of orders over 5 weeks old
# percentage_shipped_back_df['Percentage Over 5 Weeks Old'] = (percentage_shipped_back_df['Outstanding Over 5 Weeks'] / percentage_shipped_back_df['Total Orders']) * 100
#
# # Round the percentage to two decimal places
# percentage_shipped_back_df['Percentage Over 5 Weeks Old'] = percentage_shipped_back_df['Percentage Over 5 Weeks Old'].round(2)
#
# # Select and rename columns for display
# percentage_shipped_back_display = percentage_shipped_back_df[['Club', 'Percentage Over 5 Weeks Old', 'Shipped Back Orders']].copy()
# percentage_shipped_back_display.rename(columns={
#     'Percentage Over 5 Weeks Old': 'Percentage of Orders Over 5 Weeks Old (%)',
#     'Shipped Back Orders': 'Number of Orders Shipped Back'
# }, inplace=True)
#
# # Format the table using Pandas Styler
# styled_percentage_shipped_back = percentage_shipped_back_display.style.format({
#     'Percentage of Orders Over 5 Weeks Old (%)': "{:.2f}%",
#     'Number of Orders Shipped Back': "{:,}"
# })
#
# # Display the styled table
# st.write(styled_percentage_shipped_back)
#
# # -------------------------- Plot Total Open Orders Over 5 Weeks Old -------------------------- #
# st.subheader("Trend of Total Open Orders Over 5 Weeks Old")
# st.write("This graph shows the total number of open orders over five weeks old across all clubs for each report date.")
