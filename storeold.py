...
# Allow user to upload report files
uploaded_files = st.file_uploader("Upload Report CSV Files", type='csv', accept_multiple_files=True)
...
if uploaded_files:
    ...
    if not df_list:
        st.error("No valid data loaded. Please upload files with correct filenames.")
        st.stop()
    ...
    if start_date > end_date:
        ...
        st.write("This graph shows the number of open orders over five weeks old for the selected club and time period.")
        ...
        # Analysis 1: Orders Becoming Over 5 Weeks Old Between Report Dates
        st.subheader("Orders Becoming Over 5 Weeks Old Between Report Dates")
        st.write("This analysis shows the number of orders that became over five weeks old between each report date for the selected club.")
        ...
        for i in range(1, len(sorted_report_dates)):
            ...
            # Fix the error by converting dates to pandas Timestamp
        ...
        # Analysis 3: Top Clubs Contributing to New Over 5 Weeks Orders
        if selected_club == 'All Clubs':
            st.subheader("Top Clubs Contributing to New Over 5 Weeks Orders")
            st.write("This analysis shows the clubs contributing most to the number of new orders over five weeks old.")
            ...
            st.write(f"""
            For {selected_club}, the data indicates changes in the number of orders becoming over five weeks old. By comparing the number of new overdue orders with the orders shipped and new orders added, we can assess the club's order processing efficiency. If the number of orders becoming overdue is higher than the number of orders shipped, it suggests a backlog is forming. Conversely, if more orders are shipped than are becoming overdue, it indicates progress in reducing the backlog.
            """)

        # Age Distribution of Open Orders Over 5 Weeks
        st.subheader("Age Distribution of Open Orders Over 5 Weeks")
        st.write("This histogram displays the distribution of ages (in days) of open orders over five weeks old.")
    ...
    st.write("Please upload at least one report CSV file to proceed.")
