...
st.write("""
This application analyzes aggregated order data from the `Capelli2023_aggregated_orders.csv` file. Explore delivery times and shipping performance across different clubs based on the order creation dates.
""")
...
def load_data(filepath):
    ...
    df['Latest Ship Date'] = pd.to_datetime(df['Latest Ship Date'], errors='coerce')

    # Calculate 'Time to Ship' as the difference in days between 'Latest Ship Date' and 'First Order Date'
    df['Time to Ship'] = (df['Latest Ship Date'] - df['First Order Date']).dt.days
...
# Load the data
data_file = os.path.join('shippingdates', 'Capelli2023_aggregated_orders.csv')  # Update the path if necessary
...
if selected_club == 'All Clubs':
    ...
    if not filtered_df['First Order Date'].isna().all():
        within_5_weeks_df = filtered_df.dropna(subset=['First Order Date', 'Time to Ship']).copy()
        ...
        if within_5_weeks_df.empty:
            ...
            within_5_weeks_df['Creation Month'] = within_5_weeks_df['First Order Date'].dt.to_period('M').dt.strftime('%Y-%m')
            ...
            # Calculate percentages using the round() function
            within_summary['% Shipped Within 5 Weeks'] = round(
                within_summary['Orders Shipped Under 5 Weeks'] / within_summary['Total Orders Shipped'] * 100, 2
            )

            within_summary['% Shipped Over 5 Weeks'] = round(
                within_summary['Orders Shipped Over 5 Weeks'] / within_summary['Total Orders Shipped'] * 100, 2
            )
        ...
        st.write("No date created data available to display the percentage and count of orders shipped within 5 weeks.")
...
# Ensure there are date created to calculate percentages
if not filtered_df['First Order Date'].isna().all():
    percentage_df = filtered_df.dropna(subset=['First Order Date', 'Time to Ship']).copy()

    if percentage_df.empty:
        st.write("No shipping time data available to display percentage analysis.")
    else:
        # Extract Creation Month as 'YYYY-MM' string
        percentage_df['Creation Month'] = percentage_df['First Order Date'].dt.to_period('M').dt.strftime('%Y-%m')
        ...
        # Calculate percentages using the round() function
        percentage_summary['% Of Orders Shipped under 5 weeks'] = round(
            percentage_summary['Orders Shipped Under 5 Weeks'] / percentage_summary['Total Orders Shipped'] * 100, 2
        )

        percentage_summary['% Of Orders Shipped over 5 weeks'] = round(
            percentage_summary['Orders Shipped Over 5 Weeks'] / percentage_summary['Total Orders Shipped'] * 100, 2
        )
        ...
        if total > 0:
            total_under_pct = round((total_under / total * 100), 2)
            total_over_pct = round((total_over / total * 100), 2)
        else:
            total_under_pct = 0.0
            total_over_pct = 0.0
...
if not filtered_df['First Order Date'].isna().all():
    count_df = filtered_df.dropna(subset=['First Order Date', 'Time to Ship']).copy()

    if count_df.empty:
        st.write("No shipping time data available to display count analysis.")
    else:
        # Extract Creation Month as 'YYYY-MM' string
        count_df['Creation Month'] = count_df['First Order Date'].dt.to_period('M').dt.strftime('%Y-%m')
...
if not filtered_df['First Order Date'].isna().all():
    shipping_time_df = filtered_df.dropna(subset=['First Order Date', 'Time to Ship'])
    ...
    if shipping_time_df.empty:
        ...
        fig_shipping_time = px.scatter(
            ...
            labels={
                'First Order Date': 'Order Creation Date',
                'Time to Ship': 'Shipping Time (days)',
                'Club Name': 'Club'
            },
            hover_data=['order_id', 'Total Products Ordered']
        )
        ...
        fig_shipping_time.add_shape(
            dict(
                type="line",
                x0=shipping_time_df['First Order Date'].min(),
                y0=35,
                x1=shipping_time_df['First Order Date'].max(),
                y1=35,
...
# Ensure there are date created to plot
if not filtered_df['First Order Date'].isna().all():
    orders_count_df = filtered_df.dropna(subset=['First Order Date']).copy()

    if orders_count_df.empty:
        st.write("No shipping data available to display orders count over time.")
    else:
        # Extract Creation Month as 'YYYY-MM' string
        orders_count_df['Creation Month'] = orders_count_df['First Order Date'].dt.to_period('M').dt.strftime('%Y-%m')

        # Group by Creation Month and count orders
        orders_per_month = orders_count_df.groupby('Creation Month').size().reset_index(name='Number of Orders Placed')
        ...
        fig_orders_over_time = px.line(
            ...
            title='Number of Orders Placed Over Time',
            labels={
                'Creation Month': 'Month',
                'Number of Orders Placed': 'Number of Orders Placed'
            },
            markers=True
        )
        ...
        fig_orders_over_time.update_layout(
            xaxis=dict(tickangle=45),
            yaxis=dict(title='Number of Orders Placed'),
            template='plotly_white'
        )
...
if not filtered_df['First Order Date'].isna().all():
    ...
    avg_shipping_time_month['Creation Month'] = avg_shipping_time_month['First Order Date'].dt.to_period('M').dt.strftime('%Y-%m')
...
if not filtered_df['First Order Date'].isna().all():
    ...
    comparison_df['Creation Month'] = comparison_df['First Order Date'].dt.to_period('M').dt.strftime('%Y-%m')
...
st.subheader("Correlation Between Number of Orders Placed and Shipping Time")
st.write("""
This analysis examines whether there's a relationship between the number of orders placed in a month and the average shipping time for those orders. Understanding this correlation can help in forecasting shipping performance based on order volumes.
""")

# Ensure there are date created to perform correlation analysis
if not filtered_df['First Order Date'].isna().all():
    correlation_df = filtered_df.dropna(subset=['First Order Date', 'Time to Ship']).copy()

    if correlation_df.empty:
        st.write("No shipping time data available to perform correlation analysis.")
    else:
        # Extract Creation Month as 'YYYY-MM' string
        correlation_df['Creation Month'] = correlation_df['First Order Date'].dt.to_period('M').dt.strftime('%Y-%m')
        ...
        correlation_coefficient = correlation_summary['Total_Orders_Placed'].corr(correlation_summary['Average_Shipping_Time'])
        ...
        fig_correlation = px.scatter(
            ...
            trendline="ols",
            title="Correlation Between Total Orders Placed and Average Shipping Time",
            labels={
                'Total_Orders_Placed': 'Total Orders Placed per Month',
        ...
        if correlation_coefficient > 0:
            interpretation = "a positive correlation", "as the number of orders placed increases, the average shipping time also tends to increase."
        elif correlation_coefficient < 0:
            interpretation = "a negative correlation", "as the number of orders placed increases, the average shipping time tends to decrease."
        else:
            interpretation = "no correlation", "there is no apparent relationship between the number of orders placed and the average shipping time."

        st.write(f"**Interpretation:** There is {interpretation[0]} between the number of orders placed and the average shipping time. Specifically, {interpretation[1]}")
...
average_order_time = filtered_df.groupby('Club Name').agg(
    Average_Shipping_Time_Days=('Time to Ship', 'mean'),
    Number_of_Orders_Shipped=('order_id', 'count')
).reset_index()

# Round the average shipping time
average_order_time['Average_Shipping_Time_Days'] = round(average_order_time['Average_Shipping_Time_Days'], 2)
...
st.markdown("---")
st.write("**Note:** This analysis is based on the data available in the `Capelli2023_aggregated_orders.csv` file. Please ensure the data is up-to-date for accurate insights.")
